...
*Implementation notes:*
  ...
  This works for any size (rectangular, asymmetric) similarity matrix, not necessarily the (square,
  symmetric) matrix one gets from computing correlations.
...
options to do this, with different trade-offs.
...
So, in the reasonable expectation that `hclust` gave us the "optimal" clustering tree, and the only
problem was the tree order, we try:

```{r, fig.show='hold'}
clusters <- stats::hclust(stats::dist(meristems), method='ward.D2')
sheatmap(meristems, show_rownames=F, show_colnames=F, cluster_rows=clusters, cluster_cols=clusters)
```
...
Since we don't have a suitable a-priori clustering, we can ask `sheatmap` to generate a clustering
for us (in the same way as for `pheatmap`, e.g. `cluster_rows=T`).

In this (default) case, `sheatmap` will use `oclust` to generate a clustering tree, which is
constrained to be compatible with the "ideal" slanted order. That is, we try replacing the simple
use of `pheatmap` with the simple use of `sheatmap`:
