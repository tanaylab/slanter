...
*Implementation notes:*
  ...
  You can provide a different `order_data`. It must have the same shape as the reordered `data`, but
  may contain any computed appropriate similarity values (typically, only non-negative values). This
  will be used to only compute the best order, and then this order will be applied to the actually
  presented data (which may contain arbitrary, including negative, values).
...
```{r, fig.show='hold'}
sheatmap(meristems, show_rownames=F, show_colnames=F, cutree_rows=5, cutree_cols=5)
```
