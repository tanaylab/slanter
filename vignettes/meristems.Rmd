...
```{r, fig.show='hold'}
sheatmap(meristems, order_data=similarity, show_rownames=F, show_colnames=F, cluster_rows=F, cluster_cols=F)
```
...
The code actually works on the square of the data. This works OK-ish if the similarity data is a
correlation and one takes -1 (perfect negative correlation) to be as strong a similarity
indicator as +1 (perfect correlation). In the likely case the data isn't like that, just make
sure it is all non-negative, where larger values indicate "more similarity" - which we have done
here by setting all negative correlations to zero.

We can show the original data while providing a different `order_data`. It must have the same
shape as the reordered `data`, but may contain any computed appropriate similarity values
(typically, only non-negative values). The order data will be used to only compute the best
order, and then this order will be applied to the actually presented data (which may contain
arbitrary, including negative, values). In particular, any clustering will still be applied to
the full data.
...
```{r, fig.show='hold'}
sheatmap(meristems, order_data=similarity, show_rownames=F, show_colnames=F, oclust_rows=F, oclust_cols=F)
```
...
```{r, fig.show='hold'}
sheatmap(meristems, order_data=similarity, show_rownames=F, show_colnames=F)
```
...
```{r, fig.show='hold'}
sheatmap(meristems, order_data=similarity, show_rownames=F, show_colnames=F, cutree_rows=5, cutree_cols=5)
```
